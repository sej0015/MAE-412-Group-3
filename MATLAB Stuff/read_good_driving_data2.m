...
imu_data = readtable('../ros_bags/driving_with_cam2/imu.csv');
...
odom_data = readtable('../ros_bags/driving_with_cam2/odom.csv');
%odom_time = table2array(odom_data(:, 'x_time'));
wheel_speed = 0;
wheel_yaw_rate = 0;

global marker_0 marker_0_pose
marker_0 = readtable('../ros_bags/driving_with_cam2/marker_0.csv');
marker_0_pose(1, 1) = 0;
marker_0_pose(2, 1) = 0;
marker_0_pose(3, 1) = 0;

global marker_1 marker_1_pose
marker_1 = readtable('../ros_bags/driving_with_cam2/marker_1.csv');
marker_1_pose(1, 1) = 0;
marker_1_pose(2, 1) = 0;
marker_1_pose(3, 1) = 0;

global marker_3 marker_3_pose
marker_3 = readtable('../ros_bags/driving_with_cam2/marker_3.csv');
marker_3_pose(1, 1) = 0;
marker_3_pose(2, 1) = 0;
marker_3_pose(3, 1) = 0;
...
% plot(x(1, :));
% hold off
% legend('yaw angle', 'marker x pose after transformation', 'vehicle x pose');
...
function marker_0_callback(msg)
    ...
    marker_0_pose = ([cos(psi), -sin(psi), 0;
                    sin(psi), cos(psi), 0;
                    0, 0, 1] * (marker_0_pose_camera)) + [x(1:2, end); 0];
end
...
function marker_1_callback(msg)
    ...
    marker_1_pose = ([cos(psi), -sin(psi), 0;
                    sin(psi), cos(psi), 0;
                    0, 0, 1] * (marker_1_pose_camera)) + [x(1:2, end); 0];
end
...
function marker_3_callback(msg)
    ...
    marker_3_pose = ([cos(psi), -sin(psi), 0;
                    sin(psi), cos(psi), 0;
                    0, 0, 1] * (marker_3_pose_camera)) + [x(1:2, end); 0];
end